    ...
    try:
        ...
        def get_datasource_tables(datasource_name: str):
            ...
            try:
                ...
                try:
                    logger.info(f"Getting tables for datasource: {datasource_name}")
                    tables = datasource.get_tables()  # 这里只获取表的基本信息
                    ...
                    logger.error(f"Error connecting to datasource {datasource_name}: {str(e)}", exc_info=True)
                    return jsonify({
                        'success': False,
                        'error': str(e)
                    }), 503
